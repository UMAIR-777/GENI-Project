...
def extract_feature_markers(lines):
    """Extract feature markers with their line numbers"""
    markers = []
    for idx, line in enumerate(lines, start=1):
        if '@Feature:' in line or '@Scenario:' in line:
            marker = line.split('@')[1].split(':')[1].strip()
            markers.append((idx, marker))
    return markers

def run_step_tests_and_evaluate_coverage(
    production_file: str = "src/workflow_generator.py",
    tests_dir: list = ["BDD/tests_dir/test/test_workflow_generator.py", "BDD/tests_dir/test/test_workflow_generator_extra.py"], 
    report_file: str = "BDD/tests_dir/C_Reports/workflow_generator_coverage_report.json"
) -> dict:
    """
    ...
    # Configure coverage with correct paths and debug enabled
    cov = coverage.Coverage(
        source=[str(src_path)],
        omit=["*/tests/*", "*/__pycache__/*"],
        branch=True,
        data_file='.coverage',  # Specify coverage data file
        debug=['trace', 'config', 'dataio', 'sys']  # Enable debug output for coverage
    )
    ...
    try:
        ...
        if isinstance(tests_dir, list):
            test_files = [project_root / t for t in tests_dir]
            for tf in test_files:
                if not tf.exists():
                    raise FileNotFoundError(f"Test file not found: {tf}")
    ...
    feature_markers = extract_feature_markers(lines)
    covered_features = set()
    for ln in executed:
        for marker_line, marker in feature_markers:
            if marker_line == ln:
                covered_features.add(marker)

    total_features = len(feature_markers)
    ...
    report = {
        ...
        "feature_coverage": list(covered_features),
        "warnings": warnings,
    }
