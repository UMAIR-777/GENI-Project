...
NODE_SCHEMA = {
    ...
    "items": {
        ...
        "required": ["id", "input", "output", "tags", "description"]
    }
}
...
def setup_database():
    ...
    # Create node_embeddings table to store node embeddings
    cur.execute("""
    ...
    # Create function for batch similar nodes retrieval
    cur.execute("""
    ...
    BEGIN
        ...
        ranked AS (
            SELECT
                ...
                ROW_NUMBER() OVER (PARTITION BY q.query_index ORDER BY (n.embedding <=> q.embedding) ASC) AS rank
...
#             "Provides insights including object detection and scene understanding.",
#             "Ideal for image classification and visual content analysis."
#         ]
#     },
#     {
#         "id": "website_scraper",
...
#             "Extracts useful information for further analysis.",
#             "Suitable for data mining and competitive research tasks."
#         ]
#     },
#     {
#         "id": "analyze_video",
...
#             "Extracts insights and generates analytical results from video data.",
#             "Useful for multimedia content analysis and automated video summarization."
#         ]
#     },
#     {
#         "id": "Ask_AI",
...
#         "tags": ["Ask AI", "Chatbot", "AI Interaction"],
#         "description": [
#             "Interacts with a language model to generate text or answer queries.",
#             "Can be used for summarization, interactive dialogue.",
#             "Enhances user interaction by providing context-aware responses."
#         ]
#     },
#     {
#         "id": "Blog_Writer",
...
#         "description": [
#             "Generates comprehensive blog posts based on provided content and parameters.",
#             "Creates engaging and targeted content for marketing or communication purposes.",
#             "Automates the writing process for consistent and creative blog outputs."
#         ]
#     },
#     {
...
#         "description": [
#             "Retrieves the transcript from a YouTube video URL, providing the text content for further analysis.",
#             "Extracts textual content effectively, serving as both 'transcript' and 'video transcript'.",
#             "Facilitates video content analysis and further processing tasks."
#         ]
#     },
#     {
#         "id": "summarize",
...
#         "description": [
#             "Summarizes lengthy text, transcripts, or content using AI models.",
#             "Condenses information while preserving key points and overall context.",
#             "Works effectively with transcript data to produce concise summaries."
#         ]
#     },
#     {
#         "id": "generate_keywords", # Added new node
#         "input": ["text", "content"],
#         "output": ["keywords"],
#         "tags": ["Keyword Generation", "AI", "keyword extraction"],
#         "description": ["Generates relevant keywords from a given text.", 
#                         "Useful for SEO, content tagging, and information retrieval."]
#     }
# ]
...
def filter_nodes_by_embedding_batch(blueprint_steps: List[Dict], nodes: List[Dict], top_k: int = 3) -> Dict[str, List[Dict]]:
    ...
    # An extra print statement that extracts and displays only the step (subtask name), input, output, and similarity score for each candidate.
    for key, candidates in similar_nodes_by_query.items():
        print(f"\nCandidates for subtask description '{key}':")
        for candidate, similarity in candidates:
            # Print only the step (or node id), input, output and similarity score.
            print(f"\nNode: {candidate['id']}, Input: {candidate['input']}, Output: {candidate['output']}, Similarity: {similarity:.4f}\n")

    # print(f"\n--- similar_nodes_by_query (Retrieved): {similar_nodes_by_query} ---\n")
    ...
    for i, step in enumerate(blueprint_steps):
        ...
        candidate_list = "\n".join([
            f"node: {node['id']}: input={node['input']}, output={node['output']}, tags={node['tags']}, description={node['description']}" 
        ...
        """
        # FINAL_SELECTION_NODE:
        # - Primary Focus: First, identify the candidate with the best or partial inputs and outputs match.
        # - If multiple nodes are viable, choose the one most contextually relevant to subtask.
        # - Return "MISSING_NODE" only if there is zero context match across all candidate nodes for the subtask.
        ...
        try:
            ...
            if isinstance(parsed_response, dict):
                if "MISSING_NODE" in parsed_response:
                    ...
                    # Filter candidate nodes based solely on their "id"
...
def create_initial_subtask_workflow(state: AgentState) -> AgentState:
    ...
    Ensure that all fields follow this exact structure.
    
    Return ONLY a JSON list of dictionaries with the keys: "step", "input", "output", "tags", "description".
    ...
    try:
        ...
        # Create a unified field for embedding generation based on the strict blueprint format
